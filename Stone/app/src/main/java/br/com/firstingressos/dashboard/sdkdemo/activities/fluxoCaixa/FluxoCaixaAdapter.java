...
public class FluxoCaixaAdapter extends ArrayAdapter<Evento> {
    ...
    public FluxoCaixaAdapter(@NonNull Context context, int resource, @NonNull ArrayList<Evento> objects) {
...
//            imageView.setImageBitmap(BitmapFactory.decodeStream(new URL(getItem(position).getImage()).openConnection().getInputStream()));
        ...
        txtHour.setText(getItem(position).getTaxa());
